 ...
 */
async function extractQuestionsWithGemini(pdfPath) {
 ...
 */
async function processExam(course, pdfPath) {
  ...
  try {
    ...
    if (existing && existing.length > 0) {
      console.log(`ℹ️ Exam already exists, skipping: ${payload.title} (${payload.type}) for course ${course.name}`);
      return;
    }
  } catch (err) {
