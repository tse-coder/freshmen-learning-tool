...
{#if show}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
		<div
			class="mx-4 w-full max-w-md transform rounded-2xl border border-white/10 bg-gradient-to-br from-gray-900/80 to-gray-800/80 p-6 shadow-2xl backdrop-blur-md"
			role="dialog"
			aria-modal="true"
		>
			<header class="mb-4 flex items-center justify-between">
				...
				<button class="text-gray-300 hover:text-white" on:click={close} aria-label="Close"
					>&times;</button
				>
			</header>
			...
			<form on:submit|preventDefault={handleAuth} class="space-y-4">
				...
				<button
					type="submit"
					class="w-full rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 p-3 text-sm font-semibold text-white shadow-md hover:opacity-95"
