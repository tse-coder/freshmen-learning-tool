<script lang="ts">
	...
	import BroadcastEditor from '../../lib/components/BroadcastEditor.svelte';
    ...
    onMount(() => {
        ...
        const currentId = $authUser?.id?.toString() || tgUser?.id?.toString();
	...
	$: chartOptions = {
		chart: {
			id: 'new-users',
			toolbar: { show: false },
			foreColor: $theme === 'dark' ? '#cbd5e1' : '#475569'
		},
	...
	function totalSignups() {
		try {
			return chartSeries?.[0]?.data?.reduce((a: number, p: any) => a + (p?.y || 0), 0) || 0;
		} catch {
			return 0;
		}
	}
...
<div class="min-h-screen px-4 pt-24 pb-10 text-gray-900 dark:text-white">
	<div class="mx-auto max-w-7xl space-y-8">
		...
		<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
			<div
				class="rounded-xl border border-gray-200 bg-white/70 p-4 shadow-sm backdrop-blur-sm lg:col-span-2 dark:border-gray-800 dark:bg-gray-900/60"
			>
				...
				<div class="mt-2 text-xs text-gray-600 dark:text-gray-400">
					Total active users: <span class="font-medium text-gray-900 dark:text-white"
						>{totalSignups()}</span
					>
				</div>
			</div>
			...
			>
				...
				<div class="mt-2 text-sm text-gray-600 dark:text-gray-400">
					Messages: {counts.feedback_messages || 0}
				</div>
			</div>
		</div>
			...
			>
				<div class="mb-3 flex items-center justify-between">
					...
					<div class="flex items-center gap-2">
						<button
							class="flex items-center gap-1 rounded-md border border-gray-300 px-2 py-1 text-xs text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800"
							on:click={exportUsersCSV}
						>
				...
				<div class="mb-3">
					<div class="relative">
						<input
							class="w-full rounded-md border border-gray-300 bg-white px-10 py-2 text-gray-900 outline-none focus:border-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
							placeholder="Search users by name, @username, or ID"
							bind:value={searchQuery}
							on:input={() => {
								clearTimeout(searchTimer);
								searchTimer = setTimeout(() => fetchUsers(1, true), 300);
							}}
						/>
						<Search class="pointer-events-none absolute top-2.5 left-3 h-5 w-5 text-gray-400" />
					</div>
				</div>
							...
							{#if usersLoading}
								<tr
									><td
										...
										colspan="4">Loading users…</td
									></tr
								>
							{/if}
							{#each usersItems as u}
								<tr
									class="cursor-pointer border-t border-gray-100 hover:bg-gray-50/60 dark:border-gray-800 dark:hover:bg-gray-800/60"
									on:click={() => openUser(u)}
								>
									<td class="px-2 py-2">
										<div class="flex items-center gap-2">
											...
											{:else}
												<div
													class="flex h-8 w-8 items-center justify-center rounded-full bg-gray-200 text-xs font-medium text-gray-700 dark:bg-gray-700 dark:text-gray-200"
												>
									...
									<td class="px-2 py-2"
										>{u.last_seen ? new Date(u.last_seen).toLocaleString() : '-'}</td
									>
				...
				<div class="mt-3 flex items-center justify-between">
					<div class="text-xs text-gray-600 dark:text-gray-400">
						Page {currentPage} of {Math.max(1, Math.ceil(totalUsers / pageSize))} • {totalUsers} users
					</div>
					<div class="flex items-center gap-2">
						<button
							class="rounded-md border border-gray-300 px-2 py-1 text-xs text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800"
							on:click={() => fetchUsers(Math.max(1, currentPage - 1), true)}
							disabled={currentPage <= 1}>Prev</button
						>
						<button
							class="rounded-md border border-gray-300 px-2 py-1 text-xs text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800"
							on:click={() => fetchUsers(currentPage + 1, true)}
							disabled={currentPage >= Math.ceil(totalUsers / pageSize)}>Next</button
						>
					</div>
				</div>
			</div>
			...
			>
				...
				<div class="divide-y divide-gray-100 dark:divide-gray-800">
					{#each data.threads as t}
						<button
							class="w-full text-left hover:bg-gray-50/60 dark:hover:bg-gray-800/60"
						...
						>
							<div class="flex items-start gap-3 px-2 py-3">
								...
								<div class="min-w-0">
									...
									<div class="mt-1 truncate text-xs text-gray-600 dark:text-gray-400">
										{t.latest_message?.message ?? 'No messages yet'}
									</div>
	...
	{#if selectedThread}
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
			<div
				class="h-[80vh] w-full max-w-2xl overflow-hidden rounded-xl bg-white text-gray-900 shadow-2xl dark:bg-gray-900 dark:text-white"
			>
				<div
					class="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-800"
				>
					<div class="font-semibold text-gray-900 dark:text-white">
						Feedback: {selectedThread.title}
					</div>
					<button
						class="rounded-md px-2 py-1 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-800"
						on:click={() => (selectedThread = null)}>Close</button
					>
				</div>
				<div class="flex h-full flex-col">
					<div class="flex-1 space-y-3 overflow-auto p-4">
						{#each threadMessages as m}
							<div
								...
								class:m-ml-auto={m.sender_type === 'admin'}
								class:bg-blue-600={m.sender_type === 'admin'}
								class:text-white={m.sender_type === 'admin'}
								class:bg-gray-100={m.sender_type !== 'admin'}
								class:text-gray-900={m.sender_type !== 'admin'}
							>
								<div class="opacity-80">{m.message}</div>
								<div class="mt-1 text-[10px] text-gray-600 opacity-80 dark:text-gray-400">
									{new Date(m.created_at).toLocaleString()} • {m.sender_type}
								</div>
							</div>
						{/each}
					</div>
					<div class="border-t border-gray-200 p-3 dark:border-gray-800">
						<div class="flex gap-2">
							<input
								class="flex-1 rounded-md border border-gray-300 bg-white px-3 py-2 text-gray-900 outline-none focus:border-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
	...
	{#if selectedUser}
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
			<div
				class="w-full max-w-xl overflow-hidden rounded-xl bg-white text-gray-900 shadow-2xl dark:bg-gray-900 dark:text-white"
			>
				<div
					class="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-800"
				>
					<div class="flex items-center gap-3">
						...
						{:else}
							<div
								class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 text-sm font-medium text-gray-700 dark:bg-gray-700 dark:text-gray-200"
							>
					...
					<button
						class="rounded-md px-2 py-1 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-800"
						on:click={() => (selectedUser = null)}>Close</button
					>
				</div>
				...
				<div class="border-t border-gray-200 p-4 dark:border-gray-800">
					...
					<div class="space-y-2">
						{#each (data.threads || []).filter((t) => t.user_id === selectedUser.id) as t}
							<div
								class="rounded-md border border-gray-200 p-3 text-sm hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800/60"
							>
